...
export class AppointmentController {
  ...
  async updateAppointment(
    ...
    try {
      ...
      return {
        ...
        driverphoto: updatedAppointment.driverphoto ? `/uploads/${updatedAppointment.driverphoto.split('/').pop()}` : null,
      };
    } catch (error) {
  ...
  async updateGatepassStatus(
    @Param('id') id: string,
    @Body() body: { gatepassApproved: boolean },
  ) {
    try {
      const updatedAppointment = await this.appointmentService.updateGatepassStatus(id, body.gatepassApproved);
      console.log('📤 Returning updated appointment:', JSON.stringify(updatedAppointment, null, 2));
      return {
        ...updatedAppointment,
        photo: updatedAppointment.photo ? `/uploads/${updatedAppointment.photo.split('/').pop()}` : null,
